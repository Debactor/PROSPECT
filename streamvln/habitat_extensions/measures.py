...
class NDTW(Measure):
    ...
    def __init__(
        ...
        split = getattr(config, "split", "val_unseen")
        repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", ".."))
        default_gt = os.path.join(repo_root, "data", "datasets", "rxr", split, f"{split}_guide_gt.json.gz")
        self.gt_path = os.environ.get("RXR_GT_PATH", default_gt)
        with gzip.open(
            self.gt_path, "rt"
        ) as f:
    ...
    def update_metric(self, *args: Any, **kwargs: Any):
        ...
        # see /VLN-CE/habitat_extensions/config/default.py to get the default value 3.0
        nDTW = np.exp(
            -dtw_distance
            / (len(self.gt_locations) * 3.0)
        )
        self._metric = nDTW
